...
class Histfit:
    def __init__(self, hist_frac_pt, frac_values, pt_values, variable = 'pt'):
    ...
    def fitGauss(self, hist_frac, frac_values):
        parameters, covariance = curve_fit(self.gauss, frac_values, hist_frac,bounds = ([0.5,0.001,-5],[2,0.3,2000000]) ) 
        mean = parameters[0]
        sigma = parameters[1]
        const = parameters[2]
    ...
    def initiate_parameters_ci(self):
        for i in range(len(self.hist_frac_pt)):
            hist_frac = self.hist_frac_pt[i]
            mean = mean_finder(hist_frac, self.frac_values)
            sigma = std_finder(hist_frac, self.frac_values)
            
            self.parameters['mean'][i] = mean
            self.parameters['sigma'][i] = sigma
            
    def store_parameters_ci(self):
        self.initiate_parameters_ci()
